...
const TIER_CONFIG: Record<string, { color: string; bg: string; label: string; icon: string }> = {
 ...
 low: { color: "text-pulse-text-secondary", bg: "border-border ", label: "Low", icon: "○" },
};
...
function RecommendationCard({
 ...
 <p className="text-sm" style={{ color: "var(--pulse-text-secondary)" }} leading-relaxed">{rec.body}</p>
 </div>
 </div>
 ...
 variant="ghost"
 size="sm"
 className="h-7 px-2 text-xs" style={{ color: "var(--pulse-text-secondary)" }} hover:text-pulse-coral"
 ...
 <span
 key={v}
 className="rounded-full bg-muted/50 px-2 py-0.5 text-[10px]" style={{ color: "var(--pulse-text-tertiary)" }}"
 >
 {v.replace(/_/g, " ")}
 </span>
 ))}
...
export default function RecommendationsPage() {
 ...
 className="text-xs" style={{ color: "var(--pulse-text-secondary)" }} hover:text-pulse-text-primary transition-colors"
 >
